...
基本运算规则从存储介质中获取（采集）数据，然后进行计算，最后将结果存储到介质中，用于一次性计算

不适合于数据挖掘和集群学习的这种迭代计算和图形挖掘计算



mapreduce 也过于复杂，在实际的工作中，不是每个人都对 MapReduce 细微的配置细节了如指掌。
...
所谓的分片，是指把大规模的的数据分配给不同的机器 / 工

MapReduce 的性能配置上花了非常多的时间。包括了缓冲大小 (buffer size），分片多少（number of shards），预抓取策略（prefetch），缓存大小（cache size）等等
...
![image-20200112210713957](image-20200112210713957.png)
...
![image-20200112210652388](image-20200112210652388.png)
...
![image-20200112210743247](image-20200112210743247.png)
...
![image-20200112210806559](image-20200112210806559.png)



# 核心概念

![image-20200112210828853](image-20200112210828853.png)
